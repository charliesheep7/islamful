...
export function generateStaticParams() {
  return games.map((game) => ({ lang: 'ar', slug: game.slug }))
}
...
export async function generateMetadata(props: {
  ...
  return {
    title: `${game.nameAr} — لعبة إسلامية مجانية | إسلامفُل`,
    description: game.descriptionAr,
    alternates: buildLanguageAlternates(`/games/${slug}`, { currentLanguage: 'ar' }),
    openGraph: {
      title: `${game.nameAr} — لعبة إسلامية مجانية | إسلامفُل`,
      description: game.descriptionAr,
      url: `https://www.islamful.com/ar/games/${slug}`,
    },
  }
}

export default async function GamePageAr(props: { params: Promise<{ slug: string }> }) {
  const { slug } = await props.params
  const game = games.find((g) => g.slug === slug)
  if (!game) notFound()
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8" dir="rtl">
      <Breadcrumbs
        items={[
          { name: 'ألعاب إسلامية', href: '/ar/games' },
          { name: game.nameAr, href: `/ar/games/${slug}` },
        ]}
      />
      ...
      <CrossToolLinks currentTool={`games/${slug}`} lang="ar" />
    </div>
  )
}
